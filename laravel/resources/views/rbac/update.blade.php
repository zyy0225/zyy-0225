...
@section('content')
    <div class="box box-primary">
        ...
        <!-- /.box-header -->
        <!-- form start -->
        <form action="/rbac/updateDo" method="post">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" name="menu_id" value="{{$model['menu_id']}}">
            <div class="box-body">
                <div class="form-group">
                    <label><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">权限名称</font></font></label>
                    <input type="text" class="form-control" name="menu_name" value="{{$model['menu_name']}}" placeholder="Menuname">
                </div>
                <div class="form-group">
                    <label><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">标识符</font></font></label>
                    <input type="text" class="form-control" name="uri" value="{{$model['uri']}}" placeholder="Uri">
                </div>
                <div class="form-group">
                  <label>父权限</label>
                  <select class="form-control" name="pid">
                  @foreach($menu as $key=>$val)
                    <option value="{{$val['menu_id']}}"
                        @if($val['menu_id'] == $model['pid'])
                            selected=""
                        @endif
                    ><?php $count = substr_count($val['path'],'-'); echo str_repeat('|--',$count); ?>{{$val['menu_name']}}</option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group">
